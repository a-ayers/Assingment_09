...
# AyersA, 2022-Dec-11, Edited load_inventory, save_inventory methods
#------------------------------------------#
        ...
        try:
            ...
            with open(file_name_track, 'w') as track_file:
                for cd in lst_Inventory:
                    ...
                    cid = cd.cd_id
                    for song in tracks:
                        row = '{},{}'.format(cid, song.get_record())
        ...
        try:
            ...
            with open(file_name_track, 'r') as track_file:
                for line in track_file:
                    data = line.strip().split(',')
                    track_cd = PC.DataProcessor.select_cd(lst_Inventory, data[0])
                    track_to_add = DC.Track(int(data[1]), data[2], data[3])
                    track_cd.add_track(track_to_add)
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
