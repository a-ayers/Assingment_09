...
# AyersA, 2022-Dec-11, Edited program to handle the addition of tracks
#------------------------------------------#
    ...
    if strChoice == 'l':
        ...
        while True:
            ...
            if cdChoice.lower() == 'a':
                ...
                print(track_info[0], track_info[1], track_info[2])
                DC.CD.add_track(cd, DC.Track(int(track_info[0]), track_info[1], track_info[2]))
                continue
            if cdChoice.lower() == 'd':
                IO.ScreenIO.show_tracks(cd)
                continue
            if cdChoice.lower() == 'r':
                to_delete = input('Please enter the number of the track to delete: ')
                DC.CD.rmv_track(cd, int(to_delete))
            #ToDo finish the remove cd section
    elif strChoice == 's':
        IO.ScreenIO.show_inventory(lstOfCDObjects)
