...
# AyersA, 2022-Dec-10, Added Code for track class (Constructors, properties, method)
#                       Adjust code for CD class, edited methods.
#------------------------------------------#
...
class Track():
    ...
    def __init__(self, position: int, title: str, length: str):
        """Set the position, title, and length of a new track"""
        try:
...
class CD:
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        for row in self.__tracks:
            if row.position == track_id:
                to_delete = row
            else:
                continue
        # TODone remove track
        if len(self.__tracks) < 1:
            print('No tracks to deleted.')
        else:
            self.__tracks.remove(to_delete)
        # TODone sort tracks
        self.__sort_tracks()
